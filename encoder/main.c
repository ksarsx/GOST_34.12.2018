...
int hex_string_to_data(const char* hex_str, uint8_t* data) {
    // Проверяем длину строки
    if (strlen(hex_str) != KUZNECHIK_BLOCK_SIZE * 2) {
        fprintf(stderr, "Ошибка: строка должна содержать ровно 32 hex-символа\n");
        return -1;
    }
...
int hex_string_to_key(const char* hex_str, uint8_t* key) {
    // Проверяем длину строки
    if (strlen(hex_str) != KEY_SIZE * 2) {
        fprintf(stderr, "Ошибка: строка должна содержать ровно 64 hex-символа\n");
        return -1;
    }
...
int main(int argc, char* argv[])
{
    ...
    char input0[KEY_SIZE * 2 + 1];  // 64 символа + '\0'

    printf("Введите 64 hex-символа ключа (без 0x и пробелов): ");
    scanf("%64s", input0);

    if (hex_string_to_key(input0, key) == 0) {
        printf("Массив key:\n{\n");
        for (int i = 0; i < KEY_SIZE; i++) {
            printf("0x%02x", key[i]);
            if (i < KEY_SIZE - 1) {
                printf(",");
                // Красивое форматирование по 8 байт в строке
                if ((i + 1) % 8 == 0) printf("\n");
            }
        }
        printf("\n}\n");
    ...
    char input[KUZNECHIK_BLOCK_SIZE * 2 + 1]; // 32 символа + '\0'

    printf("Введите 32 hex-символа (без 0x и пробелов): ");
    scanf("%32s", input);

    if (hex_string_to_data(input, data) == 0) {
        printf("Массив data:\n{");
        for (int i = 0; i < KUZNECHIK_BLOCK_SIZE; i++) {
            printf("0x%02x", data[i]);
            if (i < KUZNECHIK_BLOCK_SIZE - 1) printf(",");
        }
        printf("}\n");
    }
